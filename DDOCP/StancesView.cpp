...
void CStancesView::OnSize(UINT nType, int cx, int cy)
{
    ...
    if (IsWindow(m_userStances.GetSafeHwnd()))
    {
        ...
        // do all the sliders
        std::vector<SliderItem>::iterator si = m_sliders.begin();
        CRect rctSizer;
        ...
        if (m_userStancebuttonsMajorForm.size() > 0)
        {
            m_userStancesMajorForm.ShowWindow(SW_SHOW);
            PositionWindow(&m_userStancesMajorForm, &itemRect, maxX);
            for (size_t i = 0; i < m_userStancebuttonsMajorForm.size(); ++i)
            {
                PositionWindow(m_userStancebuttonsMajorForm[i], &itemRect, maxX);
            }
        }
        else
        {
            m_userStancesMajorForm.ShowWindow(SW_HIDE);
            CRect notVisibleRect(-100, -100, -68, -68);
            m_userStancesMajorForm.MoveWindow(notVisibleRect, FALSE);
        }
        if (m_userStancebuttonsMantle.size() > 0)
        {
            m_userStancesMantle.ShowWindow(SW_SHOW);
            PositionWindow(&m_userStancesMantle, &itemRect, maxX);
            for (size_t i = 0; i < m_userStancebuttonsMantle.size(); ++i)
            {
                PositionWindow(m_userStancebuttonsMantle[i], &itemRect, maxX);
            }
        }
        else
        {
            m_userStancesMantle.ShowWindow(SW_HIDE);
            CRect notVisibleRect(-100, -100, -68, -68);
    ...
    if (!found)
    {
        ...
        else
        {
            ...
            if (stance.HasGroup())
            {
                ...
                else if (stance.Group() == "Major Form")
                {
                    m_userStancebuttonsMajorForm.push_back(pStance);
                }
                else if (stance.Group() == "Mantle")
                {
        ...
        bool enforceSingleActive)
{
    ...
    for (size_t i = 0; i < items.size(); ++i)
    {
        if (items[i]->GetStance().Name() == stanceName)
        {
            ...
            {
                // ensure all other items in this stance group are disabled
                for (size_t j = 0; j < items.size(); ++j)
                {
                    if (j != i && items[j]->IsSelected())
                    {
                        m_pCharacter->DeactivateStance(items[j]->GetStance());
                    }
                }
            }
        }
...
{
    bool resize = false;
    StanceActivated(m_userStancebuttons, stanceName, false);        // can have many actives
...
void CStancesView::UpdateFeatEffectRevoked(
        ...
        const Effect & effect)
{
    UpdateSlidersForEffect(effect, false);
...
void CStancesView::UpdateEnhancementEffectRevoked(
        ...
        const EffectTier & effect)
{
    UpdateSlidersForEffect(effect.m_effect, false);
        ...
        const Effect & effect)
{
    ...
    UpdateSlidersForEffect(effect, true);
...
void CStancesView::DestroyAllStances()
{
    ...
    for (auto& slider : m_sliders) {
        slider.m_label->DestroyWindow();
        slider.m_slider->DestroyWindow();
    }
    m_sliders.clear();
}
...
void CStancesView::OnHScroll(UINT sbCode, UINT nPos, CScrollBar * pScrollbar)
{
            ...
            && pScrollbar != NULL)  // must not be the window scroll bar
    {
        ...
        auto slider = GetSlider(id);
        if (slider != m_sliders.end())
        {
            nPos = (*slider).m_slider->GetPos();
            if ((*slider).m_position != nPos)
            {
                (*slider).m_position = nPos;
                CString windowText;
                windowText.Format("%s: %d", (*slider).m_name.c_str(), (*slider).m_position);
                (*slider).m_label->SetWindowText(windowText);
...
void CStancesView::UpdateSlidersForEffect(const Effect & effect, bool bApply)
{
    if (effect.Type() != Effect_CreateSlider) return;
    if (bApply) {
        // need to create or increment slider
        auto slider = GetSlider(effect);
        if (slider == m_sliders.end())
            slider = CreateSliderForEffect(effect);
        slider->m_creationCount++; // count a reference
    } else {
        // need to decrement or removal slider
        auto slider = GetSlider(effect);
        if (slider != m_sliders.end()) {
            slider->m_creationCount--;
            if (slider->m_creationCount == 0) {
                slider->m_label->DestroyWindow();
                slider->m_slider->DestroyWindow();
                Utilities::vectorQuickErase(m_sliders, slider);
...
std::vector<SliderItem>::iterator CStancesView::CreateSliderForEffect(const Effect& effect) {
    m_sliders.emplace_back();

    SliderItem& slider = m_sliders.back();

    slider.CreateControls();
    slider.m_sliderControlId = m_nextSliderId;
    slider.m_name = effect.Slider();
    slider.m_creationCount = 0;  // increment outside
    slider.m_sliderMin = (int)effect.AmountVector().at(0);
    slider.m_sliderMax = (int)effect.AmountVector().at(1);
    slider.m_position = (int)effect.Amount();
    // and create the windows controls
    CString windowText;
    windowText.Format("%s: %d", effect.Slider().c_str(), slider.m_position);
    slider.m_label->Create(
        windowText,
        SS_CENTERIMAGE | WS_VISIBLE | WS_CHILD,
        CRect(0, 0, 10, 10),        // updates on OnSize
        this,
        slider.m_sliderControlId + 1);
    slider.m_slider->Create(
        WS_VISIBLE | WS_CHILD | TBS_AUTOTICKS | TBS_HORZ | TBS_BOTTOM,
        CRect(0, 0, 10, 10),        // updates on OnSize
        this,
        slider.m_sliderControlId);
    CFont* pDefaultGUIFont = CFont::FromHandle((HFONT)GetStockObject(DEFAULT_GUI_FONT));
    slider.m_label->SetFont(pDefaultGUIFont, TRUE);
    slider.m_slider->SetFont(pDefaultGUIFont, TRUE);
    slider.m_slider->SetRange(slider.m_sliderMin, slider.m_sliderMax);
    slider.m_slider->SetPos(slider.m_position);
    ...
    return m_sliders.begin() + (m_sliders.size()-1);
}

std::vector<SliderItem>::iterator CStancesView::GetSlider(const Effect & effect) {
    for (auto it = m_sliders.begin(); it != m_sliders.end(); it++) {
        if (it->m_name == effect.Slider()) {
            return it;
        }
    }
    return m_sliders.end();
}

std::vector<SliderItem>::iterator CStancesView::GetSlider(UINT controlId)
{
    for (auto it = m_sliders.begin(); it != m_sliders.end(); it++) {
...
const SliderItem * CStancesView::GetSlider(const std::string & name) const
{
    for (auto& slider : m_sliders) {
        if (slider.m_name == name) {
            return &slider;
        }
    }
    return NULL;
...
bool CStancesView::IsStanceActive(const std::string & name, WeaponType wt) const
{
    ...
    std::vector<WeaponGroup>::const_iterator wgit = m_weaponGroups.begin();
...
void CStancesView::AddToWeaponGroup(const Effect & effect)
{
    if (effect.Type() == Effect_AddGroupWeapon)
    {
        ...
        std::vector<WeaponGroup>::iterator wgit = m_weaponGroups.begin();
        while (wgit != m_weaponGroups.end()
