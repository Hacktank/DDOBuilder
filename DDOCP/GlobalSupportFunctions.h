...
size_t TrainedCount(const std::vector<TrainedFeat> & currentFeats, const std::string & featName);
const Feat & FindFeat(const std::string & featName);
const TrainedFeat * FindFeatInList(const std::vector<TrainedFeat> & featList, const std::string & featName);
